...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const navItems = [
    { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard, path: '/dashboard', hidden: profile?.subscriptionStatus === 'free' },
  ...
  if (isAdmin || profile?.email === 'mohamedmukasin@gmail.com') {
    navItems.push({ id: 'admin', label: 'Admin Panel', icon: ShieldCheck, path: '/admin' });
  }
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
      ...
      <div className="md:hidden bg-white border-b border-slate-200 p-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-2">
          {activeView !== 'dashboard' && activeView !== 'subscription' ? (
            <Link 
              to={profile?.subscriptionStatus === 'free' ? "/subscription" : "/dashboard"}
      ...
      <aside className={cn(
        ...
        <div className="p-4 border-t border-slate-100">
            <Link 
            to="/profile"
            className="w-full bg-slate-50 rounded-2xl p-4 mb-4 hover:bg-slate-100 transition-colors text-left block"
          >
          ...
          {activeView !== 'dashboard' && activeView !== 'subscription' && (
            ...
            >
              ...
              <span className="text-sm font-semibold">
                {profile?.subscriptionStatus === 'free' ? "Back to Subscription" : "Back to Dashboard"}
              </span>
            </Link>
          )}
          {children}
